...
sensor_radial_position = [1e-3, 2e-3, 3e-3, 5e-3, 10e-3];
...
folder_name = "radial_position_study_"+string(datetime('now','format', 'yyyyMMdd_HH_mm_ss'));
...
%%
B_positions = cell(length(sensor_radial_position),1);
B0 = cell(length(sensor_radial_position),1);
B_targets = cell(length(sensor_radial_position),1);


for pos = 1:length(sensor_radial_position)

    radial_displacement = sensor_radial_position(pos);
...
set(gca, "CameraPosition", [0 0.1 1])
set(gca, "View", [0 0])

plot3(B_positions{1,1}(:,1),B_positions{1,1}(:,2),B_positions{1,1}(:,3), 'om')

ll = 9;
for ii = 1:length(sensor_radial_position)
    plot3(B_positions{ii,1}(ll,1),B_positions{ii,1}(ll,2),B_positions{ii,1}(ll,3), 'om')
end

for pos = 1:length(sensor_radial_position)
    text(B_positions{pos,1}(ll,1)+0.001, B_positions{pos,1}(ll,2), B_positions{pos,1}(ll,3), sprintf('%.0f mm', sensor_radial_position(pos)*1000), 'color', 'r')
end
...
normalize_diff_list = cell(length(sensor_radial_position),1);
values_rms_list = cell(length(sensor_radial_position),1);
...
for pos = 1:length(sensor_radial_position)
    ...
    title(t1,sprintf('Rad. Pos. %.4f', sensor_radial_position(pos)))
...
%%
% it may do strange things if you have more than 1 target
x = categorical(sensor_radial_position);
...
% set(gca, 'yscale', 'log')
plot(x, normalize_diff_EIT.*ones(1,length(sensor_radial_position)), 'dk', 'MarkerFaceColor','k')
xlabel('Rad. Position / m')
ylabel('Norm. Diff.')
