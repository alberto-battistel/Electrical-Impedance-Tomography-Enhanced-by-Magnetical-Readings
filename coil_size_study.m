...
coil_radius = [1e-3, 2e-3, 5e-3];
coil_radial_distance = 0.001;
coil_vert_displacement = 1e-3;

all_coil_values = combinations(coil_radius, coil_radial_distance, coil_vert_displacement);


folder_name = "coil_size_study";
...
for ii = 1:size(all_target_values, 1)
    ...
    target_models{ii} = mk_model_target(blank_model, ...
        target_center, ...
...
%%
coils = cell(height(all_coil_values),3);
for c = 1:height(all_coil_values)
    coil_info = all_coil_values(c,:);
    ccc = make_coil_systems(coil_info, phantom);
    for ii = 1:3
        coils{c,ii} = ccc{ii};
    end
end
...
for mdl = 1:length(target_models)
    ...
    title(t1,sprintf('Elevation %.4f', all_target_values.target_z_position(mdl)))
...
%%
% it may do strange things if you have more than 1 target
x = categorical(all_target_values.target_z_position);
...
set(gca, 'yscale', 'log')
plot(x, normalize_diff_EIT, 'dk', 'MarkerFaceColor','k')
xlabel('Target Elevation / m')
ylabel('Norm. Diff.')
...
bar(x, values_rms)
set(gca, 'yscale', 'log')
plot(x, values_rms_EIT, 'dk', 'MarkerFaceColor','k')
xlabel('Target Elevation / m')
ylabel('RMS / T')
ylim([1e-10,1e-0])
hold off
...
%% tangential coils
coil_info.center = [phantom.radius + coil_info.coil_radial_distance, 0, phantom.elec_vert_position];
coil_info.center = coil_info.center + coil_info.coil_vert_displacement;
coil_info.radius = coil_info.coil_radius;
coil_info.orientation = [0, pi/2, 0];

coils{1} = make_coil(coil_info);

%% axial coils
coil_info.center = [phantom.radius + coil_info.coil_radial_distance + coil_info.coil_radius, 0, phantom.elec_vert_position];
coil_info.center = coil_info.center + coil_info.coil_vert_displacement;
