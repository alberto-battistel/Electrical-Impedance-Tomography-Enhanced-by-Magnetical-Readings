...
target_radius = [5e-3];
target_values = [0.1, 0.5, 0.9, 1.1, 2, 10];
target_x_position = [0];
...
folder_name = "target_value_study";
...
for mdl = 1:length(target_models)
    ...
    title(t1,sprintf('Value %.1f', all_target_values.target_radius(mdl)))
...
%%
% it may do strange things if you have more than 1 target
x = categorical(all_target_values.target_values);
